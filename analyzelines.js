...
const createLineObjStream = function () {
...
const createSummaryReportStream = function () {
  return new Transform({
    transform(chunk, encoding, callback) {
      ...
      this.push(
        `Summary Report - ElapsedTime: ${
          chunkData.elapsedTime
        } milisec, LengthInBytes: ${chunkData.lengthInBytes}, TotalLines: ${
          chunkData.totalLines
        }, Rate: ${calculateBytesPerSec(chunkData)} Bytes/sec \n`
      );
      callback();
    },
  });
...
const lineObjStream = createLineObjStream();
const summaryReportStream = createSummaryReportStream();
